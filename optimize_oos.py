...
def run_backtest(df_input, fast_ema, slow_ema, trend_ema, stop_loss_pct, take_profit_pct):
    df = df_input.copy()
...
for fast, slow, trend, sl, tp in product(fast_emas, slow_emas, trend_emas, stop_losses, take_profits):
    ...
    result = run_backtest(df_train, fast, slow, trend, sl, tp)
    train_results.append(result)

train_df = pd.DataFrame(train_results)

# Filter out weak / silly configs
train_df = train_df[train_df["closed_trades"] >= 25].copy()

# Rank train results
train_df = train_df.sort_values(
    by=["final_equity", "max_drawdown", "win_rate"],
    ascending=[False, False, False]
).reset_index(drop=True)

print("\nTop 10 TRAIN results:")
print(train_df.head(10))

# =========================
# TEST TOP 10 ON OOS DATA
# =========================
top_n = min(10, len(train_df))
oos_results = []
...
for i in range(top_n):
    ...
    combined = {
        ...
        "train_equity": float(row["final_equity"]),
        "train_drawdown": float(row["max_drawdown"]),
        "train_trades": int(row["closed_trades"]),
        "train_win_rate": float(row["win_rate"]),

        "test_equity": float(test_result["final_equity"]),
        "test_buy_hold_equity": float(test_result["buy_hold_equity"]),
        "test_drawdown": float(test_result["max_drawdown"]),
        "test_trades": int(test_result["closed_trades"]),
        "test_win_rate": float(test_result["win_rate"]),
        "test_avg_win": float(test_result["avg_win"]),
        "test_avg_loss": float(test_result["avg_loss"]),
    }

    oos_results.append(combined)

oos_df = pd.DataFrame(oos_results)

oos_df = oos_df.sort_values(
    by=["test_equity", "test_drawdown", "test_win_rate"],
    ascending=[False, False, False]
).reset_index(drop=True)

print("\nTop OOS results from best TRAIN configs:")
print(oos_df)

train_df.to_csv("train_optimization_results.csv", index=False)
oos_df.to_csv("oos_test_results.csv", index=False)

print("\nSaved train_optimization_results.csv")
print("Saved oos_test_results.csv")
